...
class DataValidator:
        ...
        if ref_issues:
            for r in reports:
                if r.source_name == "orders":
                    ...
                    r.quality_score = (
                        round(r.valid_rows / r.total_rows * 100, 1) if r.total_rows > 0 else 100.0
                    )

        total_quarantined = sum(r.invalid_rows for r in reports)
        overall = (
            round(sum(r.quality_score for r in reports) / len(reports), 1) if reports else 100.0
        )
            ...
            if pd.notna(row.get("unit_price")) and row["unit_price"] <= 0:
                issues.append(
                    ValidationIssue(
                        ...
                        row_index=int(idx),
                        field="unit_price",
                        value=str(row["unit_price"]),
                        rule="positive_value",
                        message="Unit price must be positive",
                    )
                )
            if pd.notna(row.get("discount_pct")) and (
                row["discount_pct"] < 0 or row["discount_pct"] > 100
            ):
                issues.append(
                    ValidationIssue(
                        source="orders",
            ...
            if pd.notna(cost) and pd.notna(retail) and cost > retail:
                issues.append(
                    ValidationIssue(
                        ...
                        row_index=int(idx),
                        field="retail_price",
                        value=f"cost={cost}, retail={retail}",
                        rule="business_rule",
    ...
    def _check_referential_integrity(self, sources: dict) -> list[ValidationIssue]:
        ...
        product_ids = (
            set(sources["products"].df["product_id"].dropna()) if "products" in sources else set()
        )
        ...
        for idx, row in orders_df.iterrows():
            ...
            if (
                product_ids
                and pd.notna(row.get("product_id"))
                and row["product_id"] not in product_ids
            ):
                issues.append(
                    ValidationIssue(
                        source="orders",
