...
class TestDatabaseLoader:
    ...
    def test_idempotent_reload(self, loaded_db, sample_customers_df, sample_suppliers_df,
                                sample_products_df, sample_orders_df, sample_returns_df, sample_shipping_df):
        dim_builder = DimensionBuilder()
        cleaner = DataCleaner()
        enricher = DataEnricher()
    ...
    def test_quality_issues_loaded(self, db_path, sample_customers_df, sample_suppliers_df,
                                    sample_products_df, sample_orders_df, sample_returns_df, sample_shipping_df):
        dim_builder = DimensionBuilder()
        cleaner = DataCleaner()
        enricher = DataEnricher()
        ...
        quality_df = pd.DataFrame([
            {"source": "orders", "row_index": 0, "field": "qty", "value": "-1", "rule": "positive", "message": "bad"},
        ])
        loader = DatabaseLoader(db_path)
